	...
	researchAnnouncementsListing: {
		...
		*/
		init: function () {
			...
			} else if (jQuery('.podcasts-page-listing').length > 0) {
				CYGNET.researchAnnouncementsListing.podcastsListing(jQuery('.submit-all-filter').serialize(), 1);
		...
		*/
		processFieldData: function (event) {
			...
			if (jQuery('.casestudy-page-listing').length > 0) {
				...
				CYGNET.researchAnnouncementsListing.podcastsListing(formData, currentPage);
		...
		*/
		...
		/*
		Podcasts Listing by AJAX
		*/
		podcastsListing: function (formData, currentPage) {
			var pageNumber = (typeof currentPage == 'undefined') ? 1 : currentPage;
			...
			jQuery.ajax({
				...
				data: {
					...
					'form_data': formData,
					'current_page': pageNumber,
					'flag': 'podcasts_listing'
				},
				success: function (response) {
					jQuery('.podcasts-container').html(response);
					return false;
				}
			});
		},
